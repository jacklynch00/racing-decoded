...
export async function GET(request: Request) {
	try {
		...
		if (!raceIdParam || !driverIdParam) {
			return NextResponse.json({ error: 'raceId and driverId parameters are required' }, { status: 400 });
		}
		...
		if (isNaN(raceId) || isNaN(driverId)) {
			return NextResponse.json({ error: 'Invalid raceId or driverId' }, { status: 400 });
		}
		...
		if (!raceResult) {
			return NextResponse.json({ error: 'No result found for this race and driver combination' }, { status: 404 });
		}

		// Get driver and race info separately
		...
		if (!driver || !race) {
			return NextResponse.json({ error: 'Driver or race not found' }, { status: 404 });
		}
		...
		const response = {
			...
			animationData: {
				...
				averageLapTime:
					formattedLapTimes.length > 0
						? formattedLapTimes.filter((lap) => lap.timeSeconds !== null).reduce((sum, lap) => sum + (lap.timeSeconds || 0), 0) /
						  formattedLapTimes.filter((lap) => lap.timeSeconds !== null).length
						: null,
			},
		};
		...
		return NextResponse.json({ error: 'Failed to fetch lap data' }, { status: 500 });
	}
}
