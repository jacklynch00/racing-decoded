#!/usr/bin/env python3
"""
Calculate F1 Driver DNA traits and store results in database
"""
import sys
import os
...
# from calculators.clutch_calculator import ClutchCalculator
...
class DNAProcessor:
    ...
    def __init__(self):
        self.calculators = {
            ...
            # 'clutch_factor': ClutchCalculator()
        }
    ...
    def clear_existing_dna_data(self):
        """Clear existing DNA data from database"""
        try:
            engine = db_manager.connect()
            
            with engine.begin() as conn:
                conn.execute("DELETE FROM drivers_dna_breakdown")
                conn.execute("DELETE FROM drivers_dna_profiles")
            
            self.logger.info("Cleared existing DNA data")
            
        except Exception as e:
            self.logger.warning(f"Could not clear existing DNA data: {e}")

def main():
    """Main processing function"""
    logger.info("Starting F1 Driver DNA calculation")
    ...
    # Process all drivers
    min_races = int(input("Minimum races required (default 15): ") or "15")
    limit = input("Limit number of drivers (press enter for all): ")
